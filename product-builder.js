...
// Transport options with volume capacities (cubic feet)
const TRANSPORT = [
  { id: 'car', label: 'Car / Sedan', icon: '🚗', capacity: 15 },
  { id: 'suv', label: 'SUV / Crossover', icon: '🚙', capacity: 40 },
  { id: 'van', label: 'Cargo Van', icon: '🚐', capacity: 250 },
  { id: 'trailer', label: 'Trailer / Box Truck', icon: '🚚', capacity: 800 }
];
...
const App = () => {
  ...
  const handleCalculate = (selectedGenre) => {
    ...
    // Calculate total volume (cubic feet)
    const topVolume = (top.specs?.volume || 3) * topsNeeded;
    const subVolume = (sub.specs?.volume || 8) * subsNeeded;
    const totalVolume = topVolume + subVolume;
    
    // Determine recommended transport
    const recommendedTransport = TRANSPORT.find(t => t.capacity >= totalVolume) || TRANSPORT[TRANSPORT.length - 1];
        ...
        {step === 4 && result && (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-bb-orange/20 to-orange-600/10 border border-bb-orange/30 rounded-2xl p-6">
              ...
              <div className="mt-4 p-4 bg-black/20 rounded-xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="text-2xl">{result.recommendedTransport?.icon || '🚐'}</span>
                  <div>
                    <div className="text-white font-medium">Transport</div>
                    <div className="text-sm text-gray-400">
                      {result.totalVolume?.toFixed(1)} ft³ total
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-bb-orange font-bold">
                    {result.recommendedTransport?.label || 'Cargo Van'}
                  </div>
                  <div className="text-xs text-gray-500">
            ...
            <div className="flex justify-center gap-4">
              <button
                onClick={() => {
                  const accList = getSelectedAccessoriesList();
                  const accText = accList.length > 0 
                    ? `\n\nAccessories:\n${accList.map(a => `• ${a.qty}× ${a.name} - $${a.lineTotal.toLocaleString()}`).join('\n')}`
                    : '';
                  const grandTotal = result.totalPrice + accessoriesTotal;
                  const subject = encodeURIComponent(`BASSBOSS Quote - ${result.top.name} + ${result.sub.name}`);
                  const body = encodeURIComponent(
`Hi,

Here's your BASSBOSS system quote:

Configuration: ${result.crowd} people, ${result.genreLabel}

Speakers:
• ${result.topsNeeded}× ${result.top.name} - $${result.topsPrice.toLocaleString()}
• ${result.subsNeeded}× ${result.sub.name} - $${result.subsPrice.toLocaleString()}${accText}

Total: $${grandTotal.toLocaleString()} (Retail)

Power: ${result.totalAmperage.toFixed(1)}A @ 120V (${result.circuitsNeeded} × 20A circuits)
Transport: ${result.totalVolume?.toFixed(1)} ft³ - ${result.recommendedTransport?.label} or larger

---
Generated by BASSBOSS Product Builder
https://tools.bassboss.com
`);
                  window.location.href = `mailto:?subject=${subject}&body=${body}`;
                }}
                className="px-6 py-3 bg-bb-orange hover:bg-yellow-500 text-black font-semibold rounded-xl transition-colors flex items-center gap-2"
              >
